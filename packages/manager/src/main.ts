...
export class BadmintonDoublesMemberGenerator implements DoublesMemberGenerator {
  ...
  constructor({
    ...
    if (members.length < courtCount * COURT_CAPACITY) {
      throw new Error(
        "The number of participants is less than the number of people who can play in a court",
      );
    }

    this.courtCount = courtCount;
    this.histories = histories;
    this.historyKeys = new Set(
      this.histories.map((history) => toHistoryKey(history.members)),
    );
  ...
  join(): BadmintonDoublesMemberGenerator {
    ...
    const baseCount = array.mode(
      Object.values(this.gameCounts).map(({ playCount }) => playCount),
    );
  ...
  #generateRandomMembers(): GameMembers {
    ...
    const generateSize = Math.min(
      this.memberCount * 10,
      this.combinationCount - this.histories.length,
    );
...
function incrementGameCount(
  ...
  gameCounts[id] = {
    playCount: (gameCounts[id]?.playCount || 0) + 1,
    baseCount: gameCounts[id]?.baseCount || 0,
  };
}

function incrementGameCounts(
  gameCounts: PlayCountPerMember,
  members: GameMembers,
): PlayCountPerMember {
